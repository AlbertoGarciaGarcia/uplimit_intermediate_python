...
def revenue_per_region(dp: DataProcessor) -> Dict:
    ...
    data_reader_gen = data_reader.__iter__()
    ...
    for row in tqdm(data_reader_gen):
        if row[constants.OutDataColNames.COUNTRY] not in aggregate:
            aggregate[row[constants.OutDataColNames.COUNTRY]] = aggregate[
                row[constants.OutDataColNames.COUNTRY]
            ] = float(0)
        aggregate[row[constants.OutDataColNames.COUNTRY]] += dp.aggregate(
